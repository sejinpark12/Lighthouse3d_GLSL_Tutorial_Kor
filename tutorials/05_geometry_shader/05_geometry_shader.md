...
버텍스 셰이더와 마찬가지로, `gl_Position`은 선택사항입니다. 하지만 다음 단계에서 보간 목적으로 사용할 수 있습니다. `gl_PrimitiveID`는 적어도 호출되는 버텍스에 대해 작성되어야 합니다. `gl_Layer`와 `gl_ViewportIndex`가 지정된다면, 프리미티브의 모든 버텍스가 동일한 값으로 작성되어야 합니다. 지오메트리 셰이더에서 `gl_ViewportIndex`나 `gl_Layer`를 지정하지 않는다면, 이 값들은 0이 됩니다.
